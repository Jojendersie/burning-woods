...
float PassedTime;
static const float3 Force = float3(0.0f, 8.0f, 0.0f);
					...
					float3 InitialDirection : TEXCOORD1, float2 LifeTimeStartTime : TEXCOORD2)
{
    ...
    // scale
    Position.xyz *= lerp(startScale, endScale, currentTimePercentage);

	float upForceFactor =  currentTime / (length(InitialDirection) + 1.0);
	Position.xyz += InitialDirection * currentTime + Force * upForceFactor;
...
float4 FirePS(VERTEXOUTPUT In) : Color0
{
	...
	return In.Color * tex2D(noiseTexture, In.Texcoord*0.01f).r * falloff * 0.1f;
};